---
title: "Macroeconometrics and Machine Learning -- Project"
author: "Marie Cervoni, Nicolas Prat, Gabriel Sklénard"
---
...
### To begin with: Penalized linear model -- Ridge Regression
``` {r }
...
### Next step: actually take into account the time-series structure of the data, and use a given number of lags of the variables

The following chunk of code is entirely commented: it is here just to reflect our process of thought
``` {r }
...
Same here: a chunk reflecting our process of thought
``` {r }
...
A function doing one entire step of cross-validation for a given p
``` {r }
...
The computation of wanted column names was no longer necessary as we select the columns before producing the pseudo-cross-sectional data, we keep it here just to illustrate the process of thought
``` {r }
...
A function doing the whole cross-validation process and producing the best model:
``` {r }
...
### Third step: get to Neural Networks
...
We adapt the previous framework to the neural networks case: first, a function doing one step of cross-validation for a given p
``` {r }
# ## One step of cross-validation
# ### A function doing a whole step of cross-validation for parameter p
...
Then, a function doing the whole cross-validation process and producing the best model:
``` {r }
...
Unfortunately, we did not manage to make this last part work:
``` {r }
