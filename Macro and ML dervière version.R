...
cross_validation_step_nn <- function(data=data_nodate_global, Y_index=1, X_indices=1:ncol(data_nodate_global), p=1, test_amount=test_amount_global, validation_proportion=0.05, epochs_step=5, batch_size_step=64) {
  print(paste("Cross-validation -- step p = ", p, " : starting", sep=""), quote=FALSE)
  # Produce pseudo-cross-sectional data, specially adapted to lag p
  res <- produce_cross_sectional_data(data, p, Y_index, X_indices)
  X_cross_sectional <- res[[1]]
  ...
  for (i_t in 1:length(validation_indices)) {
    ...
    model %>%
      layer_dense(units = 201, activation = "relu", input_shape = c(201)) %>%
...
# - test the model on the test set and compute the mean squared error
# - return the best model itself, its mean squared error (on the test set), the best p, the mean squared errors for each p
whole_training_nn <- function(data=data_nodate_global, Y_index=1, X_indices=1:ncol(data_nodate_global), p_max=p_max_global, test_amount=test_amount_global, validation_proportion=0.05, epochs_nn=5, batch_size_nn=64) {
  # Execute cross-validation steps for each p from 1 to p_max, and store the mean squared errors of each step
  mse_cv <- rep(0.0, p_max)
  for (p in 1:p_max) {
...
predict_function <- function(x) {
  ...
  # Assurez-vous que les colonnes correspondent aux noms de variables attendus par le modèle
  names(x_matrix) <- colnames(data_nodate_global[, -Y_index])  # Remplacez Y_index par l'index de votre variable dépendante
  # Prédictions du modèle
  return(predict(final_model_nn, x_matrix))
}
# Créer un explainer LIME
explainer <- lime::lime(data_nodate_global[, -Y_index], model = predict_function)

# Obtenir les explications LIME pour une observation particulière (par exemple, la première observation)
explanation <- lime::explain(explainer, newdata = data_nodate_global[1, -Y_index, drop = FALSE], n_labels = 1)

# Afficher l'importance des variables
print(explanation)
# Créer un wrapper pour la fonction de prédiction du modèle
model_wrapper <- function(x) {
  return(predict(final_model_nn, preprocess_input(x)))
}

# Créer un objet Dalex explainer
explainer <- explain(model = model_wrapper, 
                     data = as.matrix(data_nodate_global[, -Y_index]), 
                     y = as.matrix(data_nodate_global[, Y_index]))

# Obtenir les explications pour une observation particulière (par exemple, la première observation)
explanation <- predict_parts(explainer, new_observation = as.matrix(data_nodate_global[1, -Y_index, drop = FALSE]))
...
predictions <- predict(final_model_nn, X_test)

# Erreurs de prédiction
errors <- predictions - Y_test

# Visualisation des erreurs
plot(errors, main = "Erreurs de prédiction du modèle", xlab = "Observations", ylab = "Erreurs")

# Analyse des erreurs
summary(errors)

# Visualisation des prédictions par rapport aux valeurs réelles
plot(Y_test, predictions, main = "Prédictions vs Valeurs Réelles", xlab = "Valeurs Réelles", ylab = "Prédictions")

# Visualisation des résidus
residuals <- residuals(lm(errors ~ 0 + Y_test))
plot(Y_test, residuals, main = "Résidus vs Valeurs Réelles", xlab = "Valeurs Réelles", ylab = "Résidus")

# Analyse des caractéristiques des observations avec les plus grandes erreurs
max_error_indices <- order(abs(errors), decreasing = TRUE)[1:10]
observations_with_max_errors <- data_nodate_global[test_indices[max_error_indices], ]

# Affichage des caractéristiques des observations avec les plus grandes erreurs
print("Caractéristiques des observations avec les plus grandes erreurs :")
print(observations_with_max_errors)
